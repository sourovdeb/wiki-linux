...
Use `url_to_wiki` to fetch the URL, convert it to clean Markdown, and save it under `~/wiki/converted/` so wiki_ingestor picks it up on its next cycle. Use `slug` if given, otherwise let the tool derive it from the page title.

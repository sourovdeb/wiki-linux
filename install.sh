...
# Symlink schema files into the vault (WIKI_AGENT.md Phase 2 — the schema layer
# must live inside the vault so any agent opening the vault sees the contract).
for schema in CLAUDE.md AGENTS.md WIKI_AGENT.md EXPECTATIONS.md; do
  src="$PROJECT_ROOT/$schema"
  dst="$WIKI_ROOT/$schema"
  if [[ -f "$src" && ! -e "$dst" ]]; then
    ln -s "$src" "$dst"
    ok "Linked $schema into vault"
  fi
done

# Drop a raw/README.md if missing (explains Layer 1 to the user).
if [[ ! -f "$WIKI_ROOT/raw/README.md" ]]; then
  cat > "$WIKI_ROOT/raw/README.md" <<'RAWEOF'
---
...
Drop original documents here (PDFs, papers, transcripts, plain text).
The daemon reads them; the LLM writes synthesised pages into system/, user/,
and _meta/. Files in this folder are never edited by the wiki maintainer.
RAWEOF
  ok "Wrote raw/README.md"
fi
...
chmod +x "$PROJECT_ROOT/bin/wiki-doctor"
chmod +x "$PROJECT_ROOT/bin/wiki-rescue"
chmod +x "$PROJECT_ROOT/bin/wiki-openwebui"

for _bin in wiki wiki-notify wiki-welcome wiki-launcher wiki-search-widget wiki-doctor wiki-rescue wiki-openwebui; do
...
# wiki-openwebui desktop file (main chat UI).
if [[ -f "$DESKTOP_DIR/wiki-openwebui.desktop" ]]; then
  ok "wiki-openwebui.desktop already exists"
else
  cat > "$DESKTOP_DIR/wiki-openwebui.desktop" << DESKTOP
[Desktop Entry]
Type=Application
Name=Wiki Open WebUI
GenericName=Local LLM Chat (Ollama + ~/wiki RAG)
Comment=Open WebUI chat interface, pointed at local Ollama and the wiki vault
Exec=$LOCAL_BIN/wiki-openwebui --start
Icon=web-browser
Terminal=false
Categories=Network;Education;Utility;
Keywords=wiki;chat;llm;ollama;openwebui;
StartupNotify=false
DESKTOP
  ok "Created wiki-openwebui.desktop in $DESKTOP_DIR"
fi

# wiki-doctor desktop file (safety verifier).
if [[ -f "$DESKTOP_DIR/wiki-doctor.desktop" ]]; then
  ok "wiki-doctor.desktop already exists"
else
  cat > "$DESKTOP_DIR/wiki-doctor.desktop" << DESKTOP
[Desktop Entry]
Type=Application
Name=Wiki Doctor
GenericName=Wiki Safety Audit
Comment=Verify all six EXPECTATIONS.md safety guarantees still hold
Exec=$LOCAL_BIN/wiki-doctor
Icon=emblem-default
Terminal=true
Categories=Utility;System;
Keywords=wiki;safety;audit;doctor;
StartupNotify=false
DESKTOP
  ok "Created wiki-doctor.desktop in $DESKTOP_DIR"
fi

# wiki-rescue desktop file (stray-file importer).
if [[ -f "$DESKTOP_DIR/wiki-rescue.desktop" ]]; then
  ok "wiki-rescue.desktop already exists"
else
  cat > "$DESKTOP_DIR/wiki-rescue.desktop" << DESKTOP
[Desktop Entry]
Type=Application
Name=Wiki Rescue
GenericName=Import Stray Documents
Comment=Find documents saved outside watched folders and import to ~/wiki/raw/
Exec=$LOCAL_BIN/wiki-rescue
Icon=document-open
Terminal=false
Categories=Utility;Education;
Keywords=wiki;import;rescue;raw;
StartupNotify=false
DESKTOP
  ok "Created wiki-rescue.desktop in $DESKTOP_DIR"
fi
...
# ── 6b. /etc safety snapshot (for `wiki doctor` drift detection) ─────────────
step "Snapshotting /etc allowlist hashes"

if [[ -x "$PROJECT_ROOT/bin/wiki-doctor" ]]; then
  WIKI_OS_CONFIG="$CONFIG_FILE" "$PROJECT_ROOT/bin/wiki-doctor" --resnapshot \
    && ok "Snapshot written; run 'wiki doctor' any time to verify nothing drifted" \
    || warn "Could not snapshot /etc — wiki doctor will warn about it"
fi
...
echo ""
echo "  2. Verify all safety guarantees:"
echo "       wiki doctor"
echo "     Expected: '10 passed, 0 failed' once daemon + sync are enabled."
echo ""
echo "  3. Try the wiki CLI:"
echo "       wiki status                                  # daemon + git + wiki overview"
echo "       wiki new \"My first page\"                     # create a page"
echo "       wiki ask \"What does my pacman.conf do?\"      # LLM Q&A"
echo "       wiki fix \"wifi not connecting\"               # RAG troubleshooting"
echo "       wiki archwiki \"Network configuration\"        # cache an Arch Wiki page"
echo "       wiki rescue                                  # import stray docs"
echo ""
echo "  4. Two UIs:"
echo "       Obsidian:    obsidian $WIKI_ROOT  (or 'wiki open')"
echo "                    Plugins ready: obsidian-git, dataview, templater, bmo-chatbot"
echo "                    BMO is pre-wired to local Ollama (mistral)."
echo "       Open WebUI:  wiki openwebui     (first run installs ~500 MB into venv)"
echo "                    URL: http://127.0.0.1:8080"
echo "                    Mounts $WIKI_ROOT as a knowledge folder."
echo ""
echo "  5. Auto-start Open WebUI on every login:"
echo "       wiki openwebui --enable          # adds systemd user unit"
echo ""
echo "  6. Desktop launcher + global search widget:"
echo "       wiki-launcher                    # icon grid of every command"
echo "       wiki-search-widget               # floating LLM dialog (bind to Super+W)"
echo ""
echo "  7. Check the daemon log:"

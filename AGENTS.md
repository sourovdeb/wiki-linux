...
## Verification-First Mandate (New)

- Do not anticipate. Do not assume. Verify every step before proceeding.
- Before any write operation, verify the exact target repository, branch, and file path.
- Before changing logic, verify current behavior from source code and (when available) tests.
- After each change, run or describe a concrete validation step and expected result.
- If verification fails at any step, stop and report the failure clearly.
